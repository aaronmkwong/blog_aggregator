...
├── handler_feed.go          # feed-related command handlers: addfeed
...
│       └── feeds.sql        # SQLC query definitions for feeds (CreateFeed)
└── internal/
    ├── config/
...
handler_feed.go    -> handlerAddFeed: checks arguments, gets the currently logged-in user,
...
internal/database  -> SQLC-generated type-safe Go code (not for editing)
                      Provides typed interfaces to interact with database records securely